    ...
    if ($action === 'add' && validateCSRF()) {
        ...
        if ($code && $discountPercent > 0 && $discountPercent <= 100 && $limit > 0 && $expiryDate) {
            ...
            if ($existingCoupon) {
                ...
                if ($couponId) {
                    flashMessage('Kupon başarıyla eklendi.', 'success');
                } else {
        ...
        if ($couponId && $code && $discountPercent > 0 && $discountPercent <= 100 && $limit > 0 && $expiryDate) {
            ...
            if ($existingCoupon) {
                ...
                $result = $db->update('coupons', [
                    ...
                    'usage_limit' => $limit,
                    'expiry_date' => $expiryDate
                ], 'id = ? AND firm_id = ?', [$couponId, $user['firm_id']]);
                
                if ($result) {
                    flashMessage('Kupon başarıyla güncellendi.', 'success');
                } else {
        ...
        if ($couponId) {
            $result = $db->delete('coupons', 'id = ? AND firm_id = ?', [$couponId, $user['firm_id']]);
            
            if ($result) {
                flashMessage('Kupon başarıyla silindi.', 'success');
            } else {
...
$coupons = $db->fetchAll(
    "SELECT * FROM coupons WHERE firm_id = ? ORDER BY created_at DESC",
    [$user['firm_id']]
);
...
<div class="row">
        ...
        <div class="card admin-sidebar">
            <div class="card-body p-0">
                <nav class="nav flex-column">
                    ...
                    <a class="nav-link active" href="/firma_admin/coupons.php">
    ...
    <div class="col-md-9">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4><i class="fas fa-tags"></i> Kupon Yönetimi</h4>
        ...
        <?php if (empty($coupons)): ?>
            <div class="card">
                <div class="card-body text-center py-5">
                    ...
                    <h5>Henüz kupon eklenmemiş</h5>
                    <p class="text-muted">İlk kuponunuzu eklemek için yukarıdaki butona tıklayın.</p>
                </div>
            </div>
        <?php else: ?>
            <div class="row">
